...
Backend (FastAPI)
Upload server.py and the checkpoint s2_dic_abnormal_loo10.pt to Google Colab.
Install required packages:
!pip install fastapi uvicorn pyngrok
Set your ngrok authtoken (free from ngrok.com).
Run server.py in a background thread:
...
The cell will print a public URL like https://xxx.ngrok-free.dev.
Frontend
Open index.html in any browser.
Replace the apiUrl inside the <script> tag with your ngrok URL (keep /predict at the end).
Use the sliders or click a preset (Swing / Stance / Stair Climb) – the chart updates automatically with predictions from the real model.
